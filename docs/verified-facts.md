...
건수만 맞춰 보면 안 된다 — 어느 쪽이 무엇을 흘렸는지는 txid 를 맞대야 나온다.

```
service="/error"                    A 5,822건 받아 187건 · 302ms   B 187건 · 64ms
service="/order/api/pipeline<GET>"  A 5,822건 받아 206건 · 302ms   B 206건 · 71ms
service="/order/api/summary<GET>"   A 5,822건 받아 465건 · 302ms   B 465건 · 87ms
   A 에만 있는 것 0건 · B 에만 있는 것 0건 (세 건 모두)
```

**격차는 창이 넓어질수록 벌어진다.** A 는 받아 오는 총량에 비례하고,
B 는 «걸러 준 만큼» 만 보내기 때문이다.

```
창  2분: A  2,411건 ·  167ms →  74건 | B  74건 ·  45ms   3.7배
창  5분: A  5,822건 ·  331ms → 187건 | B 187건 ·  65ms   5.1배
창 10분: A 12,641건 ·  909ms → 367건 | B 367건 · 125ms   7.3배
창 30분: A 15,128건 · 1116ms → 429건 | B 429건 · 144ms   7.8배
```

**작은 구간에서는 얻을 것이 적다.** 1,600건 남짓이면 A 도 200ms 언저리다.
값이 생기는 자리는 «넓은 구간에서 몇 건을 찾을 때» 다.
B 도 창이 넓어지면 느려진다(45→144ms) — 인덱스가 없어 구간을 다 훑기 때문이다.
