...
시각 차가 크면 차트에 배지가 뜬다 ([사용자 가이드 §7.4](user-guide.md#74-점이-사라진다-싶을-때)). 서버와 PC 의 시계를 맞춘다.
...
연결을 **1초 안에 못 얻을 때**다. 로컬에서 재현한 것은 ②뿐이고 ①·③ 은 구조상 같은 결과가 나오는 경우다.
**운영(ECS 등)에서 무엇이 원인인지는 확인하지 못했다.**
...
### 3.4 NScouter 가 보여주는 방식

- 못 닿은 서버는 상단에 **이름으로** 알린다.
- 그 서버의 행은 지우지 않고 **«지난 값»** 으로 흐리게 최대 3회 이어 보여준다. 넘으면 버리고 «못 닿음» 만 남긴다.
...
NScouter 는 종류가 `tomcat`·`java`·`jboss`·`jetty`·`resin` 인 서버만 **WAS 로 본다**(counters.xml 의 javaee 타입).
`ORDER-JVM` 처럼 **시스템 이름을 종류에 넣은 서버는 WAS 로 안 잡혀** Active 탭과 «애플리케이션» 카운터에 나오지 않는다.
운영에서는 종류 자리에 시스템 이름을 쓰는 일이 흔하다 — 서비스가 안 뜨면 이것부터 본다.

> 이 한계는 스카우터가 아니라 **NScouter 쪽**에 있다. 종류 이름 목록으로 WAS 를 가르기 때문이다.
...
앱 설정(버퍼 상한·바인딩 표시·글자 크기·자동 연결 등)은 [사용자 가이드 §7](user-guide.md#7-설정-) 에 있다.
