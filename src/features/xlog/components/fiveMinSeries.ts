...
// 자정부터 23:55 까지, 아직 오지 않은 시각까지.
// 그대로 그리면 지금 시각 이후가 바닥에 붙어 **"방금 0으로 떨어졌다"** 로 읽힌다.
// 실측: 12:47 에 조회했는데 마지막 포인트가 23:55 였다 (L4:live_host_five_min_counters).
//
// **값이 없는 슬롯은 0이 아니라 null 로 온다.** 예전에는 파서가 그걸 0으로 바꿔서
// «그때 0이었다» 와 «그때 수집이 없었다» 가 구별되지 않았다 (실측: 288칸 중 값 17칸).
...
export interface TrimmedSeries {
  obj_hash: number;
  times: number[];
  /** `null` 은 그 슬롯에 수집이 없었다는 뜻이다. 0 과 다르다 */
  values: (number | null)[];
}
 ...
 * 카운터는 슬롯 288개가 전부 0이거나 null 로 온다. 이걸 구분하지 못하면
 * 바닥에 붙은 직선을 보고 고장으로 읽는다.
 */
export function hasAnyValue(list: TrimmedSeries[]): boolean {
  return list.some(s => s.values.some(v => v !== null && v !== 0));
}
