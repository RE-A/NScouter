...
//
// 이름의 경로(`/CJFW/PRD-FSCP/FDMKT-BO`)로 한 겹 더 나눌 수도 있지만,
// 실물에서 경로 깊이가 제각각이라 계층이 들쭉날쭉해진다. 한 겹으로 둔다.
 ...
 */
export function groupAgents(agents: readonly AgentObject[], query: string): AgentGroup[] {
  const needle = query.trim().toLowerCase();
  const buckets = new Map<string, AgentObject[]>();

  for (const a of agents) {
    if (!matches(a, needle)) continue;
    const key = a.obj_type || '(unknown)';
